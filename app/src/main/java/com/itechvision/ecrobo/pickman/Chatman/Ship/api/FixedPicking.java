...
public class FixedPicking {
    String TAG = "FixedPiking";
    ...
    public void post(String code, String message, JsonArray list, HashMap<String, String> params, Activity activity) {
        ...
        if (list.size() > 0){
            ...
            if(NewPickingActivity.skipvalue == false &&  Integer.parseInt(all_row_count)==0){
...
//									act.dialog_location();
                ...
                Log.e("fixedpicking"," data present all_Row count "+all_row_count);

                /* Scan next order without updating top three color badges and orderId field */
                if(BaseActivity.getPackingList()== true && Integer.parseInt(order_picking)<=1){

                    if( (Integer.parseInt(not_inspected)==0 && Integer.parseInt(short_inspected)!=0) || Integer.parseInt(all_row_count)==0)
                        act.complete = true;
                    if(!act.packing && NewPickingActivity.packData.size()>0)
                        act.printRequest();
                    act.nextProcess1();
                    act.packing = false;
