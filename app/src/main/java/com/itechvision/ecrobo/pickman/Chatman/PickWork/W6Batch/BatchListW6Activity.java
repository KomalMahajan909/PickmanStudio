...
public class BatchListW6Activity extends BaseActivity implements View.OnClickListener,AdapBatchListDaimaru.onClickCallback, CompoundButton.OnCheckedChangeListener, DataManager.BatchListcall{
    ...
    DataManager.BatchListcall batchListcall;
    ...
    String TAG = BatchListW6Activity.class.getSimpleName();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        setContentView(R.layout.activity_batch_list_w6);
...
//        eop = position;
        if (!batchList.get(position).getBatch_status().equalsIgnoreCase("2") ) {
            if (!batchList.get(position).getBatch_status().equalsIgnoreCase("1") ) {
                Intent i = new Intent(BatchListW6Activity.this, BatchBoxW6Activity.class);
...
//        Log.e("Vallll",">>>111  "+batchstatus );

        if(CommonUtilities.getConnectivityStatus(this)) {

            progress.Show();
            BatchListRequest request = new BatchListRequest(adminID, app.getSerial(), BaseActivity.getShopId(),getResources().getString(R.string.version),etDate.getText().toString(),batchstatus,batchremark);
            manager.BatchListAPI(request, batchListcall);
        } else {
    ...
    @Override
    public void onSucess(int status, BatchListResponse message) throws JsonIOException {
        progress.Dismiss();
