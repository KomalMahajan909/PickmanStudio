...
import com.itechvision.ecrobo.pickman.Chatman.Ship.api.FixedPickCategory2;
import com.itechvision.ecrobo.pickman.Chatman.Ship.api.GetPickCategory2;
import com.itechvision.ecrobo.pickman.Chatman.Ship.api.IncludeAddPacking;
import com.itechvision.ecrobo.pickman.Chatman.Ship.api.IncludeAddPrint;
...
public class IncludeShippingActivity extends BaseActivity implements View.OnClickListener,DataManager.SetboxSizeCallback,DataManager.CheckBarcodeCallback,DataManager.FixOrderCallback,MainAsynListener {
    static SlidingMenu menu;
    SlideMenu slidemenu;
    @BindView(R.id.actionbar) ActionBar actionbar;
    @BindView(id.gridMain) LinearLayout maingrid;
    @BindView(id.allpick)Button Allpick;
    ...
    String TAG = IncludeShippingActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ButterKnife.bind(IncludeShippingActivity.this);
        getIDs();
        Log.d(TAG,"On Create ");
        ...
        if (i.hasExtra("orderId")){
            ...
            setBadge4(ShippingActivity.packBoxData.size());
        }
        ...
        mList = ShippingActivity.category1List;
        Log.e(TAG, "category1List     "+ShippingActivity.category1List);
        //selectAll = (CheckBox) this.findViewById(R.id.selectAllLVItems);
        lv = ((ListView) this.findViewById(R.id.listinclude));
...
//                menu.showMenu();
            ...
            case id.notif_count_yellow:
                Log.e(TAG,"packing Listtttttt   "+ BaseActivity.getPackingList());
    ...
    @OnClick (id.allpick)void allpick(){
        ...
        dialog.setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
            ...
            public void onClick(SweetAlertDialog sweetAlertDialog) {
                ...
                dialog.dismiss();
                new MainAsyncTask(IncludeShippingActivity.this, Globals.Webservice.Fixedshipping, 1, IncludeShippingActivity.this, "Form", Globals.getterList, true).execute();
        ...
        if(mList.size() > 0) {
            ...
            for (Map<String, String> map : mList){
                data.add(data.newItem()
                        .add(R.id.pck_0, map.get("barcode"))
                        .add(R.id.pck_1, map.get("product_name"))
                        .add(R.id.pck_2, map.get("quantity"))
                        .add(R.id.pck_3, map.get("code"))
                        .add(R.id.pck_4, map.get("location")));
            }
    ...
    public void inputedEvent(String buff, boolean isScaned) {
        switch (mProcNo) {
            case PROC_BARCODE:    // バーコード
                ...
                progress.Show();
                ShippingRequest req = new ShippingRequest(app.getSerial(), adminID, ID, orderId, barcode, category, "", "", "", "", "", "", "", "", "", "", "", getResources().getString(R.string.version),"");
...
//                if (isScaned) {
    ...
    void sendBoxSize(){
        ...
        ShippingRequest req = new ShippingRequest(app.getSerial(), adminID, ID, orderId, "", "",
...
*/
    ...
    protected boolean startPackingBoxActivity() {
        ...
        Log.e(TAG, "PackData  " + ShippingActivity.packBoxData);
        ...
        if (ShippingActivity.packBoxData.size() == 0) {
            return true;
        }
        if (BaseActivity.getPackingList() == true) {
            Log.e(TAG, " startPackingBoxActivityyy11111111111111133333333333    ");
    ...
    public void removeData(int status)
    {
        for (Map<String, String> map : mList) {
            ...
            if ((_b.equals(_b1.trim()) || _b1.matches("(.*)" + _b)) || (_b.equals(map.get("code").trim()))) {
                ...
                Log.e(TAG, " removePackItemmmm ");

            }
        }
        Log.e(TAG, "category1Listtttttttttt    "+mList);
    ...
    {Log.e(TAG,"sendRequesttt  "+is_scan);
        ...
        progress.Show();
        ShippingRequest req = new ShippingRequest(app.getSerial(), adminID, ID, orderId, barc, ProductList.get("category"), "","", ProductList.get("product_stock_history_id"), qty, timeTaken().toString(), status + "",auto_complete,box_flag,shipping_flag, airprint_printer, selectedPrinter, getResources().getString(R.string.version),"");
...
//      new ECZaikoClient(this).setListner(new FixedPikingNS()).fixed_shipping(ProductList, status, "false");
    ...
    public void createNewPackItem() {
        ...
        if (target <= productqnt) {
            if (ShippingActivity.packData.size() > 0) {
                String _b1 = mTarget.get("barcode");
                String _box1 = String.valueOf(ShippingActivity.mBoxNo);
                String _loc1 = mTarget.get("location");

                for (Map<String, String> map : ShippingActivity.packData) {
                    String _b = map.get("barcode");
                ...
                for (Map<String, String> map : ShippingActivity.packBoxData) {
                    String _b = map.get("barcode");
        ...
        if (repeat == false) {
            ...
            mPackItem.put("boxNo", String.valueOf(ShippingActivity.mBoxNo));
            ...
            ShippingActivity.packData.add(mPackItem);

            Log.e(TAG, " mpackItemm " + ShippingActivity.packData);
        }
        if(repeatpackBox == false){
            ...
            mPackItem.put("barcode", mTarget.get("barcode"));
            mPackItem.put("quantity", _gts(id.quantity).toString());
            ShippingActivity.packBoxData.add(mPackItem);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        Log.e("ShipActivityyyyyy", " onActivityResultttt");
...
//                    fixedRequest();
    ...
    public void updatePackBadge() {
        Log.e(TAG, " UpdatePackBadgeeeee ");
        int qtyBadge = 0;

        if (ShippingActivity.packBoxData != null)
            setBadge4(ShippingActivity.packBoxData.size());
        else
            setBadge4(0);
    }
    ...
    public void createAllPackset() {
        ...
        for (Map<String, String> map : mList) {
            ...
            if (ShippingActivity.packData.size() > 0) {
                ...
                for (Map<String, String> map1 : ShippingActivity.packData) {
                ...
                for (Map<String, String> map1 : ShippingActivity.packBoxData) {
                    String _b = map1.get("barcode");
                    String _box = map1.get("boxNo");
            ...
            Log.e(TAG, " mpackItemm " + ShippingActivity.packData);
            Log.e(TAG, " mpackItemm Pack boxxxxxx" + ShippingActivity.packBoxData);
    ...
    public void sendPrintRequest() {
        ...
        new MainAsyncTask(this, Globals.Webservice.addPrint, 1, IncludeShippingActivity.this, "Form", Globals.getterList,true).execute();
...
//      new ECZaikoClient(this).setListner(new NewPickingAddPrint()).addPrintNext(order, box, "order_id", "true","false");
    ...
    public void sendFinalRequest() {
        Log.e(TAG, " sendFinalRequest");


        Log.e(TAG, " sendFinalRequest Order id " + orderId);
        ...
        Globals.getterList.add(new ParamsGetter("getTrack","false"));
        Globals.getterList.add(new ParamsGetter("single_box","true"));
        Globals.getterList.add(new ParamsGetter("box_no",ShippingActivity.mBoxNo+""));

        StringBuffer orderSubId = new StringBuffer();
        StringBuffer qty = new StringBuffer();
        StringBuffer barcode =new StringBuffer();
        Log.e(TAG, " sendFinalRequesttttt  packData "+ShippingActivity.packData);
        for (Map<String, String> map : ShippingActivity.packData){
        ...
        new MainAsyncTask(this, Globals.Webservice.addPacking, 1, IncludeShippingActivity.this, "Form", Globals.getterList,true).execute();

    }

    @Override
    public void clearEvent() {
        if(BaseActivity.getPackingList() && ShippingActivity.packData.size()>0)
        {
    ...
    public void sendSelectedPrinter(){
        ...
        new MainAsyncTask(this, Globals.Webservice.sendPrinterRequest, 1, IncludeShippingActivity.this, "Form", Globals.getterList, true).execute();
    ...
    public void resetPackData() {
        ...
        ORDER_QTY_COUNT = 0;
        ShippingActivity.packData.clear();
        ShippingActivity.packBoxData.clear();
    ...
    public void setProductList(List<Map<String, String>> data) {
        Log.e("ShipActivity", " setProductListttt ");
        mProductList = data;
        findRepeat();
    }
        ...
        try {
            ...
            if ("0".equals(code) == true) {
                ...
                if (mRequestStatus == REQ_BARCODE){
                    ...
                    new GetPickCategory2().post(code, msg, result1, mHash, IncludeShippingActivity.this);
                } else if(mRequestStatus ==FIXED_REQ){
                    Log.e("SendLogs111",code+"  "+msg+"  >>>>>>>>>>>>>>>>>>>>>>>>>"+mRequestStatus);
                    new FixedPickCategory2().post(code,msg,result1, mHash,IncludeShippingActivity.this);
                } else if(mRequestStatus == REQ_ADDPRINT){
                    new IncludeAddPrint().post(code,msg,result1, mHash,IncludeShippingActivity.this);
                }  else if(mRequestStatus == REQ_ADDPACKING){
                    new IncludeAddPacking().post(code,msg,result1, mHash,IncludeShippingActivity.this);
                    ...
                    if (result1.size() > 0) {
                        ...
                        ShippingActivity.includenotinspected = all_row_count;
                        ShippingActivity.category1List.clear();

                        if(Integer.parseInt(ShippingActivity.includenotinspected)==0){
                            if(BaseActivity.getBoxNo()){
                                boxSize = "";
                                _sts(R.id.barcode, "");
                                _sts(R.id.quantity, "");
                                ...
                                setProc(IncludeShippingActivity.PROC_BOX);
...
//                                    nextProcess1();
                            ...
                            goback();
                            if (IncludeShippingActivity.inspectionNo == 1)
                                call();
                            else
                                removeData(IncludeShippingActivity.inspectionNo);
                ...
                new android.app.AlertDialog.Builder(this, R.style.DialogThemee)
                        ...
                        .setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                Intent in = new Intent(IncludeShippingActivity.this,LoginActivity.class);
                ...
                if (mRequestStatus == REQ_BARCODE) {
                    new GetPickCategory2().valid(code, msg, result1, mHash, IncludeShippingActivity.this);
                }
                else if(mRequestStatus ==FIXED_REQ){
                    new FixedPickCategory2().valid(code,msg,result1, mHash,IncludeShippingActivity.this);
                }
                else if(mRequestStatus == REQ_ADDPRINT){
                    new IncludeAddPrint().valid(code,msg,result1, mHash,IncludeShippingActivity.this);
                }
                else if(mRequestStatus == REQ_ADDPACKING){
                    new IncludeAddPacking().valid(code,msg,result1, mHash,IncludeShippingActivity.this);
                }
                else if (mRequestStatus == REQ_BOX){
                    setProc(PROC_BOX);
